...
extension ReviewViewModel: ReviewCollectionViewCellDelegate {
    func tapMemorizedSwitchButton(orm: VocabularyCardORM.ORM) {
        ...
        newCellModel.update()
        newCellModel.updateVocabularyCardListMemorizedStatus()
    ...
    func didPressedTipIcon() {
        ...
        pressTipVocabulary = cellModel.normalizedSource
        needReloadDate.accept(())
    }
...
private extension ReviewViewModel {
    ...
    /// 播放語音複習流程
    func playAudioReviewFlow() {
        playVocabularyAudio()
        guard isNeedPlaySentence() else { return }
        playSentenceAudio()
    }
    
    /// 播放單字
    func playVocabularyAudio() {
    ...
    func isNeedPlaySentence() -> Bool {
        return UserDefaults.standard[UserDefaultsKeys.isNeedReadSentences] ?? false
    }
    
    func playSentenceAudio() {
        guard let vocabulary = queryVocabularyCard(
            index: lastReadCardTableIndex)?.normalizedSource else {
            applyAudioMode(isEnable: false)
            return
        }
        guard let sentences = SimpleSentencesORM.ORM.get(normalizedSource: vocabulary) else {
            return
        }
        let numberOfReadSentencesSetting = UserDefaults.standard[UserDefaultsKeys.numberOfReadSentences] ?? 1
        let numberOfReadSentences = min(numberOfReadSentencesSetting, sentences.count)
        
        for index in 0 ..< numberOfReadSentences {
            let sentenceORM = sentences[index]
            guard let sentence = sentenceORM.sentence,
                  let translate = sentenceORM.translate else {
                continue
            }
            Speaker.shared.speakSequences("", language: .pause(time: 1))
            Speaker.shared.speakSequences(sentence, language: .en_US)
            Speaker.shared.speakSequences("", language: .pause(time: 1))
            Speaker.shared.speakSequences(translate, language: .zh_TW)
        }
    }
    ...
    func nextVocabulary() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1, execute: { [weak self, weak dispatchPointer] in
