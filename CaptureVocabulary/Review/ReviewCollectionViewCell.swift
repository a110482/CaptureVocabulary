...
private extension ReviewCollectionViewCell {
    ...
    func configDisplayTranslateSwitch() {
        ...
        displayTranslateSwitch.snp.makeConstraints {
            ...
            $0.left.equalTo(mainStackView.snp.right).offset(8)
        }
    ...
    func configChineseLabel() {
        chineseLabel.font = .systemFont(ofSize: 12)
        chineseLabel.text = NSLocalizedString("ReviewCollectionViewCell.hide", comment: "隱藏")
        chineseLabel.snp.makeConstraints {
            $0.centerX.equalTo(displayTranslateSwitch)
            $0.bottom.equalTo(displayTranslateSwitch.snp.top).offset(-2)
        }
    }
...
private extension ReviewCollectionViewCell {
    private func hideTranslate() {
        let title = NSLocalizedString("ReviewCollectionViewCell.showTranslate", comment: "顯示翻譯")
        translateButton.setTitle(title, for: .normal)
        translateButton.setTitleColor(UIColor(hexString: "3D5CFF"), for: .normal)
